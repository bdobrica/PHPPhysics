...
function ENG_Template ($file,$mainblock="main") {
...
function maketree($con,$block) {
	...
	while(list($k,$v)=each($con2)) {
		$patt="(".$this->block_start_word."|".$this->block_end_word.")\s*(\w+)\s*"
...
function getfile($file) {
	...
	if (is_file($file)) {
		...
		$file_text="<b>__ENG_Template fatal error: file [$file] does not exist__</b>";
	}
